...
# Configura el sistema de logs, activa el verboose con -v, sino tira error
def configurar_logger(verbose):
    level = logging.DEBUG if verbose else logging.WARNING
    logging.basicConfig(level=level, format='%(asctime)s - %(levelname)s - %(message)s')


# Lee archivos json y convierte en data para subir al servidor 
...
# Guarda la respuesta y la muestra con print
...
# Crea un socket tcp, conectar y enviar solicitud 
# Recibe datos en bloques y despues los une en chunks antes de devolver la respuesta
...
if __name__ == "__main__":
    ...
    # Validación del set para no mandar peticiones incompletas
    if action == "set":
