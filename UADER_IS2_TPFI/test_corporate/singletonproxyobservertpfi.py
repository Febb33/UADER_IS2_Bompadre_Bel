...
class ProxyServer:
    ...
    # Manejo de clientes según la acción
    def handle_client(self, conn, addr):
        ...
        try:
            ...
            # Suscripción, recibe notificaciones cuando se actualicen datos
            if action == "subscribe":
            ...
            conn.sendall(json.dumps({"Error": str(e)}).encode("utf-8"))
        # Cierra la conexion si el cliente no esta suscrito
        finally:
            if action != "subscribe":
                conn.close()
